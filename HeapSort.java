public class HeapSort {
    ...
    void heapify(int arr[], int n, int i) {ctr++;ctr++;
        int largest = i; ctr++;// Initialize largest as root
        int l = 2 * i + 1; ctr++; // left = 2*i + 1
        int r = 2 * i + 2; ctr++;// right = 2*i + 2
        ...
        if (largest != i) {
            int swap = arr[i];ctr++;
            arr[i] = arr[largest];ctr++;
            arr[largest] = swap;ctr++;

            // Recursively heapify the affected sub-tree
            heapify(arr, n, largest);ctr++;
        }
    }
